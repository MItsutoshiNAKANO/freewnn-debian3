...
*/
 ...
 /**    nameで与えられた名の変換対応表をオープンする。固定名と見なされない
        ...
        *dirnamptrに空文字列が入る。*errcodには次のエラーコードが入る。
        ...
        ファイルをオープンできない時2（同4）。*/
FILE *
trytoopen (name, dirnamptr, errptr)
     char *name, **dirnamptr;
     int *errptr;
{
      ...
      for (src = pathmeiorg; *src != NULL; src++)
        {
          ...
          if (NULL == (opened = fopen (taio_fnm, "r")))
            continue;
     ...
     int n;
{
  ...
  *(hen_iki = hen_iki_) = EOLTTR;
  ...
  /* 変数登録ここでまとめてする */
  for (lp = hen_iki_; lp < hen_iki;)
    *memptr++ = *lp++;
  for (hentourkptr = henorg; hentourkptr->name != NULL; hentourkptr++)
    {
      if (hentourkptr->regdflg == 0)
        ERRHYO (0);
     ...
     int m, flg;
{
  ...
  switch (fnnum)
    ...
    case -1:                    /* 変数 */
      ...
      *memptr++ = (henorg[hennum].constflg ? *(henorg[hennum].range) : hennum | IHENSU);
      break;
    ...
    case 43:
      {
        ...
        *memptr++ = (n = modsrcL (t1buf)) | IMODNM;
        atermscan (socp, t1buf, 3);
        err = chkL_get_int (t1buf, &stat, modesw[n].moderng);
        if (err != 0)
          ERRLIN (29);
        *memptr++ = stat;
        atermscan (socp, t1buf, 2);
     ...
     int flg;
{
  ...
  for (i = 0; henorg[i].name != NULL; i++)
    {
      if (ltrcmp (henorg[i].name, name))
        continue;
      /* found */
      if (flg & 04)
        ERRLIN (28);
      if (flg & 02 && henorg[i].regdflg != 0)
        ERRLIN (10);
      if (flg & 01 && henorg[i].curlinflg == 0 && henorg[i].constflg == 0)
        ERRLIN (5);
      henorg[i].curlinflg = 1;
      return (i);
    }
  if (henorg + i != hentourkptr)
    BUGreport (5);
